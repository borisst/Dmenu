                ...
                <form action="{{route('promotions.update', $promotion)}}" method="POST">
                    ...
                    <div class="flex flex-col justify-center space-y-2">
                        ...
                        <div>
                            ...
                            <select id="company" name="company_id"
                                ...
                                @foreach($companies as $company)
                                    <option value="{{$company->id}}" @if($promotion->company_id == $company->id) selected @endif>{{ucfirst($company->name)}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="event" name="event_id"
                                ...
                                @foreach($events as $event)
                                    <option value="{{$event->id}}" @if($promotion->event_id == $event->id) selected @endif class="capitalize">{{$event->company->name . ' - ' . ucfirst($event->name)}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" id="date" name="date" class="px-2 p-1 rounded-lg mt-2" value="{{$promotion->date}}">
                        </div>

                        <div>
                            <label for="price" class="block">Price <span class="text-gray-400 text-sm">(optional)</span></label>
                            <input type="number" id="price" name="price" class="px-2 p-1 rounded-lg mt-2" value="{{$promotion->price}}" >
                        </div>
                        ...
                        <div>
                            ...
                            <textarea rows="5" cols="20" id="description" name="description"
                                      class="w-full px-2 p-1 rounded-lg mt-2">{{$promotion->description}}</textarea>
                        </div>
                    </div>
